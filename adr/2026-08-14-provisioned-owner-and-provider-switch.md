...
Provisioners must generate a unique, high-entropy random secret, store it as a deployment secret, and construct the activation URL. Anyone possessing an unexpired setup link can race to complete owner setup, so the delivery channel remains security-sensitive. Anonymous visitors can learn that an owner has not yet set a password, but not the owner's email or anything about the secret. Obot must preserve the restricted-session authorization boundary for future APIs and must never treat deployment configuration as account recovery after setup completes.
