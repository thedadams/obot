...
Catalog and deployed-server configuration use one `config` collection, with `usage` identifying environment variables, headers, files, dynamic files, and interpolation-only values. Legacy multi-user servers mark per-user headers with `userAllowed` on their configuration items; vMCPs continue to control user input through component policies. Startup migrations flatten stored MCPServers and SystemMCPServers alongside their respective catalog entries, retaining deprecated fields only for migration. Catalog entries no longer declare `serverUserType`. Each vMCP component uses a shared runtime when it has no user-allowed non-header configuration; otherwise it uses a runtime dedicated to the user. Administrators may force dedicated operation per component.

Shared backing MCPServers belong to the vMCP; dedicated backing MCPServers belong to each vMCPInstance. Components choose their sharing modes independently. A component sharing-mode change deletes servers owned by the previous mode, and component removal deletes that component's backing servers. User-allowed headers remain instance-specific even when the backing servers are shared.

Each shared component also has an MCPServerInstance owned by the vMCPInstance. Its credential contains that connection's allowed header values, synchronized from the parent vMCPInstance credential. The aggregate connects to `/mcp-connect/<MCPServerInstanceID>` for shared components and to the dedicated MCPServer for single-user components. Signed internal requests authorize these exact connection IDs. Deleting a vMCPInstance or changing a component's sharing mode removes its obsolete connections and their credentials.
...
Access and tool grants are defined by additive user and group profiles embedded in the vMCP. New connections reuse the user's existing `vMCPInstance`; migration may preserve multiple legacy connections for the same user and vMCP. Each instance has its own credential. Clients may connect through `/mcp-connect/<vMCPInstanceID>` to select a particular connection, and the aggregate retains this identity through its internal loopback. The instance may enable any subset of the tools granted by all matching profiles, but it cannot widen that union. Profile, group, component, and policy changes reconcile existing instances and credentials.

A profile's `vmcpPermissions.allowAllComponents` grants all tools enabled on the vMCP. Otherwise, `vmcpPermissions.allowedComponents` maps component IDs to tool selections: a component's `allowedTools` value of `null` grants all enabled tools for that component, and an empty array grants no tools. An empty component map grants no access. Instances store their selections in `componentSet`. External clients connect through the vMCP so its tool restrictions apply. Backing component endpoints accept only authenticated internal forwarding scoped to those components.
...
- A user-allowed non-header input requires a dedicated runtime for that component unless the policy changes; user-allowed headers alone do not.
