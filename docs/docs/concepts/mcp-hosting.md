...
vMCPs replace the legacy standalone connection and composite server models. Existing endpoints remain available for migration compatibility in this release; Obot will migrate them in the next release, after which vMCPs will be the only gateway connection path. See [Virtual MCPs](../functionality/virtual-mcps.md) for creation, access, configuration, and migration guidance.
