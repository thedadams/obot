...
This page describes the legacy MCP server model, which remains available for compatibility in this release. Create new connection endpoints as [virtual MCPs (vMCPs)](./virtual-mcps.md). In the next release, Obot will migrate all MCP servers to vMCPs, and vMCPs will be the only way to connect through the Obot Gateway.
