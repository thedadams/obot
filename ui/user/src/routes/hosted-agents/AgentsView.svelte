<script lang="ts">
	...
	interface Props {
		...
		instances: HostedAgentInstance[];
		pools: HostedAgentPool[];
	}

	let {
		hostedAgents: initialAgents,
		instances: initialInstances,
		pools: initialPools
	}: Props = $props();
	let hostedAgents = $state<HostedAgent[]>(untrack(() => initialAgents));
	let instances = $state<HostedAgentInstance[]>(untrack(() => initialInstances));
	let pools = $state<HostedAgentPool[]>(untrack(() => initialPools));
...
<div class="flex h-full w-full flex-col gap-5" in:fade={{ duration }}>
