...
test('selects, applies, restores and removes server names containing commas', async () => {
	...
	await expect.element(select).toMatchTextContent(name);
	...
	await expect.element(select).toMatchTextContent('Other server');
	...
	await select.click({ position: { x: 5, y: 5 } });
	await page.getByRole('button', { name, exact: true }).click();
	await expect.element(select).not.toMatchTextContent(name);
	await expect.element(select).toMatchTextContent('Other server');
	await select.click({ position: { x: 5, y: 5 } });
	await page.getByRole('button', { name, exact: true }).click();
	await select.getByRole('textbox').click();
	await userEvent.keyboard('{Backspace}');
	await expect.element(select).not.toMatchTextContent('Other server');
	await expect.element(select).toMatchTextContent(name);
