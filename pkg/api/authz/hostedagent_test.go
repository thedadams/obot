...
func TestEntersSandbox(t *testing.T) {
	for _, tt := range []struct {
		...
		{
			pattern: "GET /api/hosted-agent-instances/{hosted_agent_instance_id}/terminal",
			want:    true,
		},
		{
			pattern: "/agent-connect/{hosted_agent_instance_id}",
			want:    true,
		},
		{
			pattern: "/agent-connect/{hosted_agent_instance_id}/{rest...}",
			want:    true,
		},

		// Managing the record, which both roles keep.
		{
			pattern: "GET /api/hosted-agent-instances",
			want:    false,
		},
		{
			pattern: "GET /api/hosted-agent-instances/{hosted_agent_instance_id}",
			want:    false,
		},
		{
			pattern: "PUT /api/hosted-agent-instances/{hosted_agent_instance_id}",
			want:    false,
		},
		{
			pattern: "DELETE /api/hosted-agent-instances/{hosted_agent_instance_id}",
			want:    false,
		},

		// A route that merely mentions an agent is not a way into one.
		{
			pattern: "GET /api/hosted-agents/{hosted_agent_id}",
			want:    false,
		},
		{
			pattern: "GET /api/hosted-agent-pools/{hosted_agent_pool_id}/utilization",
			want:    false,
		},
	} {
...
func TestRevealsSecrets(t *testing.T) {
	for _, tt := range []struct {
		...
		{
			pattern: "POST /api/hosted-agents/{hosted_agent_id}/reveal",
			want:    true,
		},
		...
		{
			pattern: "PUT /api/hosted-agents/{hosted_agent_id}",
			want:    false,
		},
		{
			pattern: "GET /api/hosted-agents",
			want:    false,
		},
	} {
		t.Run(tt.pattern, func(t *testing.T) {
